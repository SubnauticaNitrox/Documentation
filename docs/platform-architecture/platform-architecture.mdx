...
### {vars.projects.server.name}

Server application that is started by the {vars.projects.launcher.name} project. It has serialization logic for Subnautica game entities which makes it easier to communicate with Nitrox clients. A future plan is to abstract away serialization to custom Nitrox objects instead of Subnautica game objects.

### {vars.projects.launcher.name}
