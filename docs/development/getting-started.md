...
See [setting up a local git repository](/docs/development/contributing/git-workflow#fork-the-nitrox-repo-on-github-to-your-own-account) and [the tools we use](/docs/development/contributing/tools).
...
1. Run Nitrox server (if not already started) and verify that server window shows no (critical) errors.
...
:::note
   There will probably be some errors, but nothing substantial in the first part regarding loading of Nitrox.
:::
